...
def _line_segments_inside_frame(frame, point, direction, include_boundary=False):
    ...
    for a, b in zip(unique, unique[1:]):
        ...
        on_boundary = include_boundary and _frame_boundary_clearance_2d(frame, test) <= 1.0e-7
        if _frame_contains_2d(frame, test) or on_boundary:
            segments.append((a, b))
...
def _mapped_line_intervals(frame, point, direction, common_origin, common_direction):
    """Clip a parallel line and express its intervals on the common-line axis."""
    mapped = []
    for lo, hi in _line_segments_inside_frame(frame, point, direction):
        p0 = point + direction * lo
        p1 = point + direction * hi
        t0 = (p0 - common_origin).dot(common_direction)
        t1 = (p1 - common_origin).dot(common_direction)
        mapped.append((min(t0, t1), max(t0, t1)))
    return _merge_intervals(mapped)


def _solid_surface_extent(source_frame, receiver_frame, common, lo, hi):
    """Use the longer intersection across the receiver's two solid surfaces.

    A penetrating plate tilted inside its own plane can intersect the top and
    bottom of a thick receiver at different endpoint positions.  A cutter
    based only on the receiver center plane then leaves a triangular corner.
    Intersect the source center plane with both real receiver surfaces, clip
    against both outlines, and return their full shared t extent.
    """
    origin = common["origin"]
    axis = common["direction"].normalized()
    candidates = [(lo, hi)]
    eps = 1.0e-7
    for surface_d in receiver_frame["surface_ds"]:
        point, direction = _line_plane_intersection(
            source_frame["n"], source_frame["center_d"],
            receiver_frame["n"], surface_d,
        )
        if point is None:
            continue
        src = _mapped_line_intervals(source_frame, point, direction, origin, axis)
        dst = _mapped_line_intervals(receiver_frame, point, direction, origin, axis)
        for a0, a1 in src:
            for b0, b1 in dst:
                x0, x1 = max(a0, b0), min(a1, b1)
                if x1 - x0 <= eps:
                    continue
                # Keep the surface interval belonging to this center overlap;
                # do not bridge holes or unrelated disconnected islands.
                if x1 >= lo - eps and x0 <= hi + eps:
                    candidates.append((x0, x1))
    return min(a for a, _b in candidates), max(b for _a, b in candidates)

def _edge_face_contact(frame_a, frame_b, scene, settings, allow_boundary=False):
    ...
    for i, p in enumerate(outer):
        ...
        for surface_d in frame_b["surface_ds"]:
            ...
            # An edge coincident with the receiver outline is a valid open
            # edge-insert when enabled: the DIFFERENCE helper then straddles
            # the outline and creates a slot open to the sheet boundary.
            clipped = _line_segments_inside_frame(
                frame_b, projected_p, ed, include_boundary=allow_boundary
            )
...
def _try_edge_face_contact(frame_a, frame_b, scene, settings, allow_boundary=False):
    try:
        return _edge_face_contact(frame_a, frame_b, scene, settings, allow_boundary)
    except Exception:
        return None


def _resolve_edge_insert_roles(obj_a, obj_b, frame_a, frame_b, scene, settings, allow_boundary=False):
    """Automatically decide edge/tooth plate versus face/slot plate."""
    ab = _try_edge_face_contact(frame_a, frame_b, scene, settings, allow_boundary)
    ba = _try_edge_face_contact(frame_b, frame_a, scene, settings, allow_boundary)
    ...
    if (not allow_boundary
            and chosen[4]["clearance"] <= mm_to_bu(scene, settings.joint_detect_tolerance_mm) * 0.25
            and other["clearance"] <= mm_to_bu(scene, settings.joint_detect_tolerance_mm) * 0.25):
...
def _make_helper_mesh(scene, joint_id, target, operation, name, verts, faces):
    ...
    # Boolean operands must be valid closed meshes.  Blender 5.2's Exact
    # solver may merely imprint edges (without changing volume) when an
    # operand has inverted winding, so helpers are constructed outward below
    # and validated here before they enter the modifier stack.
...
def _make_box_helper(scene, joint_id, target, operation, name, center, ax, ay, az, dims):
    ...
    # Outward winding for the index layout above.  1.7.0 used the exact
    # reverse of every face, which broke DIFFERENCE booleans in Blender 5.2.
    faces = [(3,2,1,0),(5,6,7,4),(1,5,4,0),(2,6,5,1),(3,7,6,2),(7,3,0,4)]
...
def _make_prism_helper(scene, joint_id, target, operation, name, polygon, normal, thickness):
    if len(polygon) < 3 or thickness <= 1.0e-10:
        raise RuntimeError(f"连接棱柱尺寸无效：{name}")
    normal = normal.normalized()
    # Normalize polygon winding so the cap/side convention below is outward
    # regardless of plate normal, selection order, or contact direction.
    polygon = list(polygon)
    poly_n = Vector((0.0, 0.0, 0.0))
    origin = polygon[0]
    for i in range(1, len(polygon) - 1):
        poly_n += (polygon[i] - origin).cross(polygon[i + 1] - origin)
    if poly_n.length <= 1.0e-12:
        raise RuntimeError(f"连接棱柱轮廓退化：{name}")
    if poly_n.dot(normal) < 0.0:
        polygon.reverse()
    n = normal * (thickness * 0.5)
...
def _generate_edge_insert(scene, settings, record, params, a, b, fa, fb):
    allow_boundary = bool(params.get(
        "allow_boundary_insert", getattr(settings, "joint_allow_boundary_insert", True)
    ))
    tooth_obj, slot_obj, tooth_frame, slot_frame, contact = _resolve_edge_insert_roles(
        a, b, fa, fb, scene, settings, allow_boundary
    )
...
def _generate_cross(scene, settings, record, params, a, b, fa, fb):
    ...
    for idx, (lo, hi) in enumerate(common["overlaps"], 1):
        ...
        extent_for_a = _solid_surface_extent(fb, fa, common, lo, hi)
        extent_for_b = _solid_surface_extent(fa, fb, common, lo, hi)
        ...
        for half_index, (t0, t1) in enumerate(halves):
            ...
            extent = extent_for_a if cut_a else extent_for_b
            if half_index == 0:
                t0 = min(t0, extent[0])
            else:
                t1 = max(t1, extent[1])
...
def _generate_through(scene, settings, record, params, a, b, fa, fb):
    ...
    for i, (lo, hi) in enumerate(common["overlaps"], 1):
        cut_lo, cut_hi = _solid_surface_extent(fp, fr, common, lo, hi)
        length = cut_hi - cut_lo
        if length <= eps:
            continue
        center = common["origin"] + d * ((cut_lo + cut_hi) * 0.5)
        helper = _make_box_helper(
...
class FLATFAB_OT_load_joint_params(Operator):
    ...
    def execute(self, context):
        ...
        if hasattr(settings, "joint_allow_boundary_insert"):
            settings.joint_allow_boundary_insert = bool(params.get(
                "allow_boundary_insert", settings.joint_allow_boundary_insert
            ))
...
class FLATFAB_PG_Settings(PropertyGroup):
    ...
    joint_allow_boundary_insert: BoolProperty(
        name="允许板面边界开放插槽",
        default=True,
        description="边插接触线位于接收板轮廓边缘时也生成插齿，并在接收板生成向边缘敞开的插槽",
    )
    ...
    def draw(self, context):
        ...
        if opened:
            ...
            if settings.joint_type == "EDGE_INSERT":
                box.prop(settings, "joint_allow_boundary_insert")
                box.separator()
